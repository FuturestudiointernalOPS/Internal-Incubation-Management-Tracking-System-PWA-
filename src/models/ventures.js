 ...
 *
 * Completion is measured against the steps that define required content
 * (1 Startup Identity, 2 Business Information, 3 Founder Information,
 * 4 Team Information). Each such step carries an equal share (25%).
 *
 * Steps without required fields (5 Supporting Documents — optional, and
 * 6 Review & Submit — no data of its own) never add credit, so an untouched
 * profile is 0% and a profile with every required step filled reaches ~100%
 * even before any optional documents are uploaded.
 */
export function calculateCompletion(profileData) {
  if (!profileData) return 0;

  // Steps that carry required content; optional/review-only steps are excluded.
