...
export default function TaskManager({
  ...
  const handleAddTask = useCallback(async () => {
    ...
    if (data.success) {
      setForm({
        name: "", project_id: "", category: "",
        start_date: "", due_date: "", start_time: "", due_time: "",
      });
  ...
  // ── Open sub-task form ──
  const openSubTask = useCallback((parentId, parentProjectId, parentCategory) => {
  ...
  const activeTasks = useMemo(() =>
    tasks.filter((t) =>
      !["completed", "archived", "carried_over"].includes(t.status) && !t.parent_task_id
    ),
  [tasks]);
  ...
  const renderTaskRow = (task, isSub = false) => {
    ...
    return (
      <div key={task.id}>
        <div className={`flex items-center gap-2 py-1.5 ${isSub ? "ml-6 pl-3 border-l-2 border-indigo-500/30" : ""}`}>
          {/* Checkbox */}
          <button
            onClick={() => updateStatus(task.id, task.status === "completed" ? "in_progress" : "completed")}
            disabled={isUpdating}
          ...
          {!isSub && task.subtasks?.length > 0 ? (
            ...
            >
              <span className={`text-[11px] font-medium ${task.status === "completed" ? "line-through text-slate-500" : "text-[var(--text-primary)]"} ${isSub ? "text-[10px]" : ""}`}>
                {task.title}
              </span>
              <ChevronDown className={`w-3 h-3 shrink-0 text-slate-500 transition-transform ${isExpanded ? "rotate-180" : ""}`} />
            </button>
          ) : (
            <span className={`flex-1 text-[11px] font-medium min-w-0 truncate ${task.status === "completed" ? "line-through text-slate-500" : "text-[var(--text-primary)]"} ${isSub ? "text-[10px]" : ""}`}>
              {task.title}
            </span>
          )}
          ...
          {/* Sub-task count */}
          {!isSub && task.subtasks?.length > 0 && (
            <span className="text-[8px] text-indigo-400 font-bold shrink-0">{task.subtasks.length} sub</span>
          )}
        </div>
        ...
        {!isSub && (
          <button
            onClick={() => openSubTask(task.id, task.project_id, task.category)}
  ...
  return (
    <div className={cn("space-y-4", compact ? "text-sm" : "")}>
      ...
      {/* Empty state */}
      {activeTasks.length === 0 && carryOverTasks.length === 0 && !showTaskForm && (
        ...
        <div className="p-3 rounded-xl border border-[var(--brand-orange)]/30 bg-[var(--brand-orange)]/[0.02] space-y-3">
          ...
          {!pendingParentTaskId && (
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="relative">
                <label className="text-[7px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Project</label>
                {form.project_id ? (
                    ...
                    {showProjectDropdown && (
                      <div className="absolute z-10 mt-1 w-full max-h-32 overflow-y-auto rounded-lg bg-[var(--bg-primary)] border border-[var(--border-primary)] shadow-xl">
                        {filteredProjects.length === 0 ? (
                          <p className="px-3 py-2 text-[9px] text-slate-500 italic">No projects</p>
                        ) : (
                          filteredProjects.map((p) => (
                            <button
                              key={p.id}
                              onClick={() => { setForm((f) => ({ ...f, project_id: p.id, category: "" })); setProjectSearch(""); setShowProjectDropdown(false); }}
              ...
              <div>
                <label className="text-[7px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Category</label>
                <select
                  value={form.category}
                  onChange={(e) => setForm((p) => ({ ...p, category: e.target.value, project_id: e.target.value ? "" : p.project_id }))}
          ...
          {pendingParentTaskId && form.project_id && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
              <span className="text-[8px] font-bold text-indigo-400">Inherited from parent</span>
          ...
          <div className="flex gap-2">
            <button onClick={handleAddTask}
              disabled={!form.name.trim() || (!form.project_id && !form.category)}
            ...
            <button onClick={() => { setShowTaskForm(false); setPendingParentTaskId(null); }}
