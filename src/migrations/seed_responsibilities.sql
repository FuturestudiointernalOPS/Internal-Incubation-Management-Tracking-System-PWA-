...
INSERT INTO responsibilities (name, key, description, icon) VALUES
  ...
  ('Operations', 'operations', 'Internal operations — workspace, reports, standups', 'Settings'),
  ('CRM', 'crm', 'People, contacts, timeline, forms, communications', 'Users'),
...
WHERE role = 'super_admin'
  AND r.key IN ('engineering', 'program_management', 'project_ownership', 'operations',
                'crm', 'finance', 'reporting', 'knowledge_base',
...
FROM contacts, responsibilities r
WHERE role = 'staff'
  AND r.key IN ('engineering', 'project_ownership', 'crm', 'reporting')
ON CONFLICT DO NOTHING;
...
-- Program Manager gets program management + crm + reporting
INSERT INTO user_responsibilities (user_cid, responsibility_id, assigned_by)
SELECT cid, r.id, 'seed'
FROM contacts, responsibilities r
WHERE role = 'program_manager'
  AND r.key IN ('program_management', 'crm', 'reporting')
ON CONFLICT DO NOTHING;

-- Teacher gets program management + crm
INSERT INTO user_responsibilities (user_cid, responsibility_id, assigned_by)
SELECT cid, r.id, 'seed'
FROM contacts, responsibilities r
WHERE role = 'teacher'
  AND r.key IN ('program_management', 'crm')
ON CONFLICT DO NOTHING;
