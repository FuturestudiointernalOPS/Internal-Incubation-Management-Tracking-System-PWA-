...
import { Loader2, Search, ArrowUpDown, Eye, FileText, BarChart3 } from "lucide-react";
...
export default function ResponsesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const all = [];
      for (const run of runsList.filter(r => !["draft", "cancelled"].includes(r.status))) {
        try {
  ...
  const filtered = allSubs
    .filter(s => {
      ...
      if (search) {
        ...
        // Also search in submission data values (startup name, email, etc.)
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-[var(--text-secondary)] mt-1">
              {allSubs.length} submissions across {runs.filter(r => !["draft","cancelled"].includes(r.status)).length} active runs
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs font-bold text-[var(--text-secondary)] uppercase">No submissions yet</p>
            <p className="text-[10px] text-[var(--text-secondary)] mt-1 opacity-50">Launch a run and share the link to start collecting responses</p>
          </div>
        ) : (
          <table className="w-full text-left">
            <thead className="sticky top-0 bg-secondary z-10">
              <tr className="text-[9px] font-black uppercase text-[var(--text-secondary)] border-b border-[var(--border-primary)]">
                ...
                <th className="px-4 py-3 hidden md:table-cell">Run</th>
                <th className="px-4 py-3 w-16 text-center">Score</th>
                <th className="px-4 py-3 w-20">Status</th>
                <th className="px-4 py-3 w-24 hidden lg:table-cell">Submitted</th>
            ...
            <tbody className="divide-y divide-[var(--border-primary)]">
              {filtered.map(s => {
                ...
                return (
                  <tr
                    key={s.id}
                    className="hover:bg-tertiary/30 transition-colors cursor-pointer"
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[10px] text-[var(--text-secondary)] hidden md:table-cell">{formName(s.form_id)}</td>
                    <td className="px-4 py-3 text-[10px] text-[var(--text-secondary)] hidden md:table-cell">{runName(s.run_id)}</td>
                    ...
                    <td className="px-4 py-3 text-[10px] text-[var(--text-secondary)] hidden lg:table-cell">
