...
export default function ParticipantDashboard() {
  ...
  useEffect(() => {
    const handleLangChange = () =>
      setLang(localStorage.getItem("impactos-lang") || "en");
    window.addEventListener("impactos:languageChange", handleLangChange);
    handleLangChange(); // Initial load
    return () =>
      window.removeEventListener("impactos:languageChange", handleLangChange);
  }, []);
  ...
  const fetchData = async (userData) => {
    try {
      ...
      const res = await fetch(`/api/pm/full-state`);
      const data = await res.json();
      if (data.success) {
        setCurriculum(data.sessions || []);
      }

      let subUrl = `/api/participant/submissions?participant_id=${userData.id}`;
      if (isTeam) {
        subUrl = `/api/participant/submissions?team_id=${teamId}`;
      }
  ...
  const validateSubmission = () => {
    ...
    if (format === "link" || format === "url") {
      ...
      if (!urlPattern.test(submissionValue)) {
        setValidationError(
          "ERROR: VALID URL PROTOCOL REQUIRED (e.g. https://...)",
        );
        return false;
      }
    }
  ...
  const handleSendSignal = async () => {
    ...
    try {
      const res = await fetch("/api/internal-comms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sender_id: user.id,
  ...
  const calculateProgress = () => {
    ...
    const completedWeight =
      submissions.filter((s) => s.status === "reviewed").length * 5; // Simplified for demo
    return Math.min(
      100,
      Math.round((completedWeight / (totalWeight || 1)) * 100),
    );
  };
        ...
        <header className="flex flex-col lg:flex-row justify-between items-center gap-10 bg-white/[0.02] border border-white/5 p-12 rounded-[3rem] shadow-2xl">
          <div className="space-y-4">
            ...
            <h2 className="text-6xl font-black text-white tracking-tighter uppercase italic leading-none">
              {user.role === "team" ? user.name : t("navigation.progress")}
            </h2>
            <p className="text-slate-400 font-bold max-w-xl opacity-70">
              Complete requirements to increase your progress and unlock the
              next phase.
            </p>
          </div>
            ...
            <div className="relative w-48 h-48 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle
                  ...
                  strokeDasharray={502}
                  strokeDashoffset={502 - (502 * calculateProgress()) / 100}
                />
              </svg>
            ...
            <div className="space-y-6">
              {curriculum.map((session) => {
                const isCompleted = submissions.some((s) =>
                  s.requirement_id.startsWith(session.id),
                ); // Simplified check
                return (
                  <div
                    key={session.id}
                    className="ios-card bg-white/[0.01] border-white/5 p-8 hover:bg-white/[0.02] transition-all group overflow-hidden relative"
                  >
                    <div className="flex justify-between items-center relative z-10">
                      <div className="flex items-center gap-8">
                        <div
                          className={`w-16 h-16 rounded-2xl flex items-center justify-center font-black text-2xl italic border transition-all ${isCompleted ? "bg-[#FF6600] text-black border-[#FF6600] shadow-[0_0_20px_rgba(255,102,0,0.4)]" : "bg-white/5 text-slate-700 border-white/5"}`}
                        >
                          {isCompleted ? (
                        ...
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            <span className="text-[9px] font-black text-[#FF6600] uppercase tracking-widest italic">
                              {session.assignment_type || "Workshop"}
                            </span>
                            <div className="w-1 h-1 rounded-full bg-white/20" />
                            <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
                              {session.weight} Units
                            </span>
                          </div>
                        ...
                        {(() => {
                          ...
                          return (
                            <a
                              href={mUrl}
                              target="_blank"
                              className="p-4 rounded-2xl bg-white/5 text-slate-500 hover:text-white transition-all flex items-center gap-3"
                            >
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-6">
              {submissions.slice(0, 5).map((sub) => (
                <div
                  key={sub.id}
                  className="ios-card bg-white/[0.01] border-white/5 p-6 space-y-4"
                >
                  <div className="flex justify-between items-start">
                    <span
                      className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${sub.status === "reviewed" ? "bg-[#FF6600]/20 text-[#FF6600]" : "bg-white/5 text-slate-600"}`}
                    >
                      {sub.status}
                    </span>
                  ...
                  <p className="text-sm font-bold text-white leading-relaxed">
                    {sub.report_body ||
                      "Submission successful. No tactical report provided."}
                  </p>
                  {sub.feedback && (
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-[11px] font-black text-[#FF6600] uppercase tracking-[0.5em] italic">
                        Tactical Deployment
                      </span>
                    </div>
                  ...
                  <button
                    ...
                    className="p-5 bg-white/5 rounded-3xl text-slate-600 hover:text-white transition-all transform hover:rotate-90"
                  >
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest ml-2 italic">
                      Submission Content (
                      {activeSession.allowed_format || "General"})
                    </label>

                    {activeSession.allowed_format?.toLowerCase() === "link" ||
                    activeSession.allowed_format?.toLowerCase() === "url" ? (
                      ...
                      <div
                        onClick={() =>
                          setSubmissionValue(
                            "https://storage.impactos.com/mock-upload.pdf",
                          )
                        }
                      ...
                      >
                        <div
                          className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all ${submissionValue ? "bg-[#FF6600] text-black" : "bg-white/5 text-slate-600 group-hover:text-[#FF6600] group-hover:scale-110"}`}
                        >
                          {submissionValue ? (
                            <CheckCircle2 className="w-8 h-8" />
                          ) : (
                        ...
                        <p
                          className={`text-[11px] font-black uppercase tracking-widest ${submissionValue ? "text-[#FF6600]" : "text-slate-700"}`}
                        >
                          {submissionValue
                            ? "Node Cached Successfully"
                            : `Click to Upload ${activeSession.allowed_format || "File"}`}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest ml-2 italic">
                      Tactical Status Report (Optional)
                    </label>
                    <textarea
                      rows={4}
                  ...
                  <button
                    onClick={() =>
                      handleSubmit(activeSession.id, activeSession.program_id)
                    }
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  ...
                  <button
                    ...
                    className="p-4 bg-white/5 rounded-2xl text-slate-600 hover:text-white transition-all transform hover:rotate-90"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                <div className="p-6 bg-[#FF6600]/5 border border-[#FF6600]/20 rounded-2xl flex items-start gap-4">
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed text-left">
                    Your inquiry will be routed directly to the designated
                    Program Authority. Please provide clear tactical context.
                  </p>
                </div>

                <div className="space-y-6">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest ml-2 italic text-left">
                      Subject / Objective
                    </label>
                    <input
                  ...
                  <div className="space-y-3">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setSignalData({ ...signalData, body: e.target.value })
                      }
                    />
                  </div>
                </div>
