    ...
    <DashboardLayout role="participant" activeTab="projects">
      <div className="max-w-6xl mx-auto space-y-6 pb-16">
            ...
            <div className="w-full lg:w-64 space-y-2">
              ...
              <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${metrics.percentComplete || 0}%` }}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0 ${week.locked ? "bg-white/5 text-slate-700" : week.current ? "bg-[#FF6600]/15 text-[#FF6600]" : "bg-emerald-500/10 text-emerald-400"}`}
                        >
                          {week.locked ? (
                        ...
                        <div className="min-w-0">
                          <p
                            className={`text-xs font-black uppercase truncate ${week.locked ? "text-slate-600" : "text-white"}`}
                          >
                            {week.title}
                          </p>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {!week.locked && (
                          <ChevronDown
                            className={`w-3.5 h-3.5 text-slate-600 transition-transform ${selectedWeek === week.number ? "rotate-180" : ""}`}
                          />
                        )}
                      </div>
                    </div>
              ...
              {selectedWeek ? (
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {deliverables
                      .filter((d) => d.week_number === selectedWeek)
                      .map((del) => {
                        const status = getDeliverableStatus(del.id);
                        ...
                        return (
                          <div
                            key={del.id}
                            className={`ios-card bg-[#0F172A] border !p-5 flex items-center justify-between gap-4 transition-all ${isLocked ? "border-white/5 opacity-50" : status === "approved" || status === "completed" ? "border-emerald-500/20 bg-emerald-500/[0.02]" : "border-white/5 hover:border-[#FF6600]/20"}`}
                          >
                            <div className="flex items-center gap-4 min-w-0 flex-1">
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-base flex-shrink-0 ${status === "approved" || status === "completed" ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-slate-500"}`}
                              >
                                {isLocked ? (
                ...
                <div className="ios-card bg-[#0F172A] border-white/5 !p-12 text-center">
                  ...
                  <p className="text-sm text-slate-400">
                    Click a week from the timeline to view its deliverables and
                    sessions.
                  </p>
                </div>
              )}
                      ...
                      .map((doc) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              onClick={() => handleSubmitLink(doc.id)}
        ...
        {activeTab === "messages" && (
          <div className="space-y-6">
            <h2 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3">
              <MessageSquare className="w-4 h-4 text-[#FF6600]" /> Messages
            </h2>

            {/* INCOMING: PM Followups */}
            <div>
              <h3 className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-500" /> Incoming
              ...
              <div className="space-y-3">
                {followups.length === 0 ? (
                  ...
                  followups.map((fol) => (
                    <div
                      key={fol.id}
                      className="ios-card bg-[#0F172A] border-white/5 !p-5 border-l-4 border-l-emerald-500 hover:bg-white/[0.03] transition-all"
                    >
                      <div className="flex items-center gap-3 mb-2">
            ...
            {/* OUTGOING: Send Feedback */}
            <div>
              <h3 className="text-[10px] font-black text-[#FF6600] uppercase tracking-widest mb-3 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[#FF6600]" /> Outgoing —
                send to Program Manager
              </h3>
              <div className="ios-card bg-[#0F172A] border-white/5 !p-6 text-center">
                ...
                <p className="text-xs text-slate-400 max-w-md mx-auto mb-5">
                  Share your learnings, challenges, or suggestions with your
                  Program Manager.
                </p>
                <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="ios-card bg-[#0F172A] border-white/5 !p-5">
                ...
                <div className="mt-2 text-[8px] text-slate-500 font-semibold uppercase tracking-widest border-t border-white/5 pt-2">
                  Week {currentWeek} of{" "}
                  {program.duration_weeks || sessions.length || currentWeek}
                </div>
              </div>
            </div>
                  ...
                  <p className="text-lg font-black text-[#FF6600]">
                    {
                      deliverables.filter(
                        (d) =>
                          ...
                          !submissions.find((s) => s.deliverable_id === d.id),
                      ).length
                    }
                  </p>
                ...
                {sessions.slice(0, 6).map((ses) => (
                  <div key={ses.id} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-2.5 h-2.5 rounded-full ${ses.status === "completed" ? "bg-emerald-500" : "bg-[#FF6600]"}`}
                      />
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-2">
                    Share your thoughts with the Program Manager.
                  </p>
                </div>
                <button
