  ...
  try {
    ...
    if (existing.rows.length > 0) {
      ...
      await db.execute({
        sql: `UPDATE v2_op_reports SET
          ...
          WHERE id = ?`,
        args: [
          reportData.completed_work, reportData.unfinished_tasks,
          reportData.challenges, reportData.wins,
          reportData.carryover_items, reportData.week_status, reportData.retro_notes,
          reportData.had_blockers, reportData.blocker_type,
          reportData.blocker_desc, reportData.major_achievement,
          reportId,
        ],
      });
    } else {
    ...
    if (reconciliation && Array.isArray(reconciliation)) {
      for (const item of reconciliation) {
        ...
        if (!['completed', 'carried_over', 'in_progress'].includes(status)) continue;
        ...
        try {
          ...
          reconciledTasks.push({ id: parseInt(task_id), status, success: true });
        } catch (e) {
          reconciledTasks.push({ id: parseInt(task_id), status, success: false, error: e.message });
        }
      }
    }
