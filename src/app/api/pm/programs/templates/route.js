...
export async function POST(req) {
  try {
    ...
    if (action === "save") {
      ...
      await db.execute({
        sql: `INSERT INTO v2_programs
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'template')`,
        args: [
          templateId, template_name, p.description, p.concept_note, p.vision,
          p.objectives, p.program_type || "incubation", p.visibility || "private",
          p.participant_limit || 0, p.registration_window, p.language || "en",
          p.duration_weeks || 4, p.grading_mode || "graded",
          p.feedback_enabled != null ? p.feedback_enabled : 1, p.materials,
        ],
      });
    ...
    if (action === "apply") {
      const { template_id, name, start_date, end_date, assigned_pm_id } = await req.json();
      if (!template_id || !name) {
        return NextResponse.json(
      ...
      const newId = "P-" + new Date().getFullYear() + "-" + uuidv4().split("-")[0].toUpperCase();
      await db.execute({
        sql: `INSERT INTO v2_programs
          (id, name, description, concept_note, vision, objectives, program_type, visibility,
           participant_limit, registration_window, language, duration_weeks, grading_mode,
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Planned', ?)`,
        args: [
          newId, name, t.description, t.concept_note, t.vision, t.objectives,
          t.program_type || "incubation", t.visibility || "private",
          t.participant_limit || 0, t.registration_window, t.language || "en",
          t.duration_weeks || 4, t.grading_mode || "graded",
          t.feedback_enabled != null ? t.feedback_enabled : 1, t.materials,
          start_date || null, end_date || null, assigned_pm_id || null, t.id,
        ],
      });
