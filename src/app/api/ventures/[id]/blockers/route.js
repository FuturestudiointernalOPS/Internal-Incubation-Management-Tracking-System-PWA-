...
export async function GET(req, { params }) {
  try { await initDb(); const authError = await requireAuth(ROLES); if (authError) return authError;
    ...
    const r = await db.execute({ sql: `SELECT b.*, c.name as creator_name FROM blockers b LEFT JOIN contacts c ON b.user_id = c.cid WHERE b.venture_id = ? ORDER BY b.created_at DESC`, args: [id] });
...
export async function POST(req, { params }) {
  try { await initDb(); const authError = await requireAuth(ALLOWED); if (authError) return authError;
    const { id } = await params; const { session } = await requireVentureAccess(id, db);
    if (!session) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
    const { title, description, venture_retro_id, task_id } = await req.json();
    ...
    const task = await db.execute({ sql: "SELECT id FROM tasks WHERE id = ? AND venture_id = ?", args: [task_id, id] });
    ...
    await db.execute({
      sql: "INSERT INTO blockers (task_id, title, description, venture_id, venture_retro_id, status, user_id, user_name) VALUES (?,?,?,?,?,'active',?,?)",
      args: [task_id, title, description||null, id, venture_retro_id, session.cid, contact.rows?.[0]?.name || ""],
    });
...
export async function PATCH(req, { params }) {
  try { await initDb(); const authError = await requireAuth(ALLOWED); if (authError) return authError;
    ...
    if (action === "resolve") {
      const b = await db.execute({ sql: "SELECT user_id FROM blockers WHERE id = ? AND venture_id = ?", args: [blocker_id, id] });
