...
export async function GET(req, { params }) {
  try { await initDb(); const authError = await requireAuth(ROLES); if (authError) return authError;
    ...
    const tasksRes = await db.execute({ sql: "SELECT COUNT(*) as total, SUM(CASE WHEN status='done' THEN 1 ELSE 0 END) as done FROM tasks WHERE venture_id = ?", args: [id] });
    const total = parseInt(tasksRes.rows?.[0]?.total||0);
    const done = parseInt(tasksRes.rows?.[0]?.done||0);

    const milestonesRes = await db.execute({ sql: "SELECT AVG(progress) as avg_progress FROM venture_milestones WHERE venture_id = ?", args: [id] });

    const standupsRes = await db.execute({ sql: "SELECT COUNT(*) as count FROM venture_standups WHERE venture_id = ?", args: [id] });
    const retrosRes = await db.execute({ sql: "SELECT COUNT(*) as count FROM venture_retros WHERE venture_id = ?", args: [id] });
