...
async function autoCalc(ventureId, source) {
  if (source === "customer_interviews") {
    const r = await db.execute({ sql: "SELECT COUNT(*) as c FROM venture_customer_interviews WHERE venture_id = ?", args: [ventureId] });
    return parseInt(r.rows?.[0]?.c || 0);
  }
  if (source === "milestones") {
    const r = await db.execute({ sql: "SELECT AVG(progress) as avg_progress FROM venture_milestones WHERE venture_id = ?", args: [ventureId] });
...
export async function PATCH(req, { params }) {
  try {
    ...
    await db.execute({ sql: "UPDATE venture_kpi_assignments SET current_value = ?, updated_at = NOW() WHERE id = ? AND venture_id = ?", args: [current_value, assignmentId, id] });
    return NextResponse.json({ success: true });
  } catch (e) {
    return NextResponse.json({ success: false, error: e.message }, { status: 500 });
  }
