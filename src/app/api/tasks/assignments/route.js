...
export async function GET(req) {
  try {
    ...
    let sql = "SELECT ta.*, t.title as task_title, t.project_id FROM task_assignments ta JOIN tasks t ON ta.task_id = t.id WHERE 1=1";
    const args = [];
    if (assignee_id) { sql += " AND ta.assignee_id = ?"; args.push(assignee_id); }
    if (status) { sql += " AND ta.status = ?"; args.push(status); }
...
export async function POST(req) {
  try {
    ...
    if (!assignment_id || !action) {
      return NextResponse.json({ success: false, error: "assignment_id and action required" }, { status: 400 });
    }
    ...
    const a = inv.rows[0];
    if (a.status !== "pending") {
      return NextResponse.json({ success: false, error: "Assignment no longer pending" }, { status: 400 });
    }

    const userCid = session.cid;
    if (a.assignee_id !== userCid) {
      return NextResponse.json({ success: false, error: "Only the assignee can respond" }, { status: 403 });
    }
    ...
    if (action === "decline") {
      ...
      await db.execute({
        ...
        args: [a.assigner_id, "Assignment Declined", `${session.name || userCid} declined task "${task?.title || '#' + a.task_id}"`, "task_assignment"],
      });
    ...
    if (action === "accept") {
      ...
      await db.execute({
        sql: "INSERT INTO v2_notifications (recipient_id, title, message, type, is_read) VALUES (?, ?, ?, ?, 0)",
        args: [a.assigner_id, "Assignment Accepted", `${session.name || userCid} accepted task "${task?.title || '#' + a.task_id}"`, "task_assignment"],
      });
