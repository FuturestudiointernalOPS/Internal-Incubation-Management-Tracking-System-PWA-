...
export async function GET(req) {
  try {
    ...
    const programResult = await getVentureProgramKpiFields(ventureId);
    if (programResult.rows.length === 0) return NextResponse.json({ success: true, kpis: [] });
    ...
    const kpis = [
      { kpi_key: "completion", kpi_label: "Program Completion", kpi_value: `${Number(programRow.completion_index || 0).toFixed(0)}%`, trend: Number(programRow.completion_index || 0) > 50 ? "up" : "stable" },
      ...
      { kpi_key: "status", kpi_label: "Venture Status", kpi_value: programRow.status || "active", trend: "stable" },
    ];
