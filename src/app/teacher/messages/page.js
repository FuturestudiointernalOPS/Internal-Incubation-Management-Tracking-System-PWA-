...
export default function TeacherMessages() {
  ...
  const fetchContacts = useCallback(async () => {
    if (!user?.cid && !user?.id) return;
    try {
      // Teacher can message: super_admin, their program's PM, and participants
      const res = await fetch(`/api/teacher/full-state?cid=${user.cid || user.id}`);
      const data = await res.json();
      const allowed = [];

      // Add SA + PMs from contact list
      const contactRes = await fetch("/api/contacts");
      const contactData = await contactRes.json();
      if (contactData.success) {
        const admins = contactData.contacts.filter(
          (c) => c.status === "active" && ["super_admin", "program_manager"].includes(c.role),
        );
        allowed.push(...admins);
      }

      // Add participants from teacher's programs
      if (data.teams) {
        data.teams.forEach((team) => {
          if (team.members) {
            team.members.forEach((m) => {
              if (!allowed.find((a) => (a.cid || a.id) === (m.cid || m.id))) {
                allowed.push({ ...m, role: "participant" });
              }
            });
          }
        });
