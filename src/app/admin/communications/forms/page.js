...
import { Plus, CheckSquare, AlignLeft, CheckCircle, Search, Save, X, Link as LinkIcon, Loader2, Copy, Edit3, Trash2 } from 'lucide-react';
  ...
  const deleteForm = async (formId) => {
    ...
    try {
      ...
      if (data.success) {
        fetchForms();
        window.dispatchEvent(new CustomEvent('impactos:notify', { 
           detail: { type: 'success', message: 'Form archived successfully.' } 
        }));
      } else {
        window.dispatchEvent(new CustomEvent('impactos:notify', { 
           detail: { type: 'error', message: data.error || 'Failed to archive form.' } 
        }));
      }
    } catch (e) {
      console.error(e);
              ...
              <div key={f.form_id} className="ios-card group hover:border-[#FF6600]/30 transition-all duration-300">
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-2">
                     ...
                     <button onClick={() => deleteForm(f.form_id)} className="p-2 rounded-lg bg-white/5 border border-white/5 text-slate-500 hover:text-rose-500 transition-all" title="Archive Form">
                        <Trash2 className="w-4 h-4" />
                     </button>
                     <span className="badge badge-glow-success uppercase text-[8px] font-black h-fit ml-2">ACTIVE</span>
                  </div>
                </div>
