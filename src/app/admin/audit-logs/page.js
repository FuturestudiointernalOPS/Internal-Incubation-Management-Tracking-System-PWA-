      ...
      <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-1">{t("adminMisc.auditLogs.subtitle")}</p>
          </div>
          <button
            onClick={refreshAll}
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-xl p-4">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.events24h")}</p>
            </div>
            {(stats.by_severity || []).map((severityStat) => (
              <div key={severityStat.severity} className="bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-xl p-4">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{severityStat.severity}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Filters */}
        <div className="bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-xl p-4 mb-6">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg text-sm font-bold text-[var(--text-primary)] placeholder-[var(--text-tertiary)] focus:outline-none focus:border-[var(--brand-orange)]"
              />
            </div>
            <select
              value={filters.event_type}
              onChange={(event) => setFilters((previous) => ({ ...previous, event_type: event.target.value, offset: 0 }))}
              className="px-4 py-2.5 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg text-sm font-bold text-[var(--text-primary)] focus:outline-none focus:border-[var(--brand-orange)]"
            >
        ...
        {loading ? (
          ...
          <div className="bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-xl p-12 text-center">
            <Info className="mx-auto mb-3 text-[var(--text-secondary)]" size={40} />
            <p className="text-sm text-[var(--text-secondary)]">{t("adminMisc.auditLogs.emptyState")}</p>
            <p className="text-sm text-[var(--text-tertiary)] mt-2">{t("adminMisc.auditLogs.emptyStateDesc")}</p>
          </div>
        ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border-primary)]">
                    <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.colTimestamp")}</th>
                    <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.colEventType")}</th>
                    <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.colActor")}</th>
                    <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.colDescription")}</th>
                    <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.colSeverity")}</th>
                    <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{t("adminMisc.auditLogs.colActions")}</th>
                  </tr>
                </thead>
                <tbody>
                  {logs.map((log) => {
                    ...
                    return (
                      <tr
                        key={log.id}
                        className="border-b border-[var(--border-secondary)] hover:bg-[var(--surface-2)] transition-colors cursor-pointer"
                        onClick={() => setSelectedLog(log)}
                      >
                        <td className="p-4 text-sm font-bold text-[var(--text-primary)] whitespace-nowrap">
                        ...
                        <td className="p-4">
                          <div className="text-sm font-bold text-[var(--text-primary)]">{log.actor_name || log.actor_cid}</div>
                          {log.actor_role && <div className="text-[10px] font-medium text-[var(--text-secondary)]">{log.actor_role}</div>}
                        </td>
            ...
            <div className="flex items-center justify-between p-4 border-t border-[var(--border-primary)]">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 hover:bg-[var(--surface-2)] rounded-lg disabled:opacity-30 transition-colors"
                >
                  <ChevronLeft size={16} />
                </button>
                <button
        ...
        {selectedLog && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setSelectedLog(null)}>
            <div className="bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-xl w-full max-w-2xl max-h-[80vh] overflow-y-auto m-4" onClick={(event) => event.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b border-[var(--border-primary)]">
                <h2 className="text-lg font-black tracking-tight flex items-center gap-2">
                  <Shield size={18} className="text-[var(--brand-orange)]" />
                  {t("adminMisc.auditLogs.detailsTitle")}
                </h2>
                <button onClick={() => setSelectedLog(null)} className="p-2 hover:bg-[var(--surface-2)] rounded-lg">
                  <X size={16} />
                </button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.colEventType")}</p>
                    <p className="text-sm font-bold">{selectedLog.event_type?.replace(/_/g, " ")}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.colSeverity")}</p>
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.colTimestamp")}</p>
                    <p className="text-sm font-bold">{formatDate(selectedLog.created_at)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.colActor")}</p>
                    <p className="text-sm font-bold">{selectedLog.actor_name || selectedLog.actor_cid}</p>
                    {selectedLog.actor_role && <p className="text-[10px] font-medium text-[var(--text-secondary)]">{selectedLog.actor_role}</p>}
                  </div>
                  {selectedLog.venture_id && (
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.venture")}</p>
                      <p className="text-sm font-bold">{selectedLog.venture_id}</p>
                    </div>
                  )}
                  {selectedLog.ip_address && (
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.ipAddress")}</p>
                      <p className="text-sm font-bold font-mono text-[var(--text-primary)]">{selectedLog.ip_address}</p>
                    </div>
                  )}
                </div>
                {selectedLog.description && (
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.colDescription")}</p>
                    <p className="text-sm font-bold text-[var(--text-primary)]">{selectedLog.description}</p>
                  </div>
                )}
                {selectedLog.metadata && typeof selectedLog.metadata === "object" && Object.keys(selectedLog.metadata).length > 0 && (
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] mb-1">{t("adminMisc.auditLogs.metadata")}</p>
