...
export default function AdminCampaignsPage() {
  ...
  const handleCreate = async () => {
    if (!form.venture_id || !form.name) {
      setToast({ type: "error", message: "Venture and campaign name are required" });
      return;
    }
    try {
      ...
      if (data.success) {
        setToast({ type: "success", message: "Campaign created (Draft)" });
        setShowCreate(false);
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setToast({ type: "success", message: `Campaign ${newStatus}` });
        fetchData();
      } else {
        setToast({ type: "error", message: data.error });
      }
  ...
  const handleUpdateRaised = async (id) => {
    const amount = prompt("Enter current amount raised (USD):");
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setToast({ type: "success", message: "Amount updated" });
        fetchData();
      } else {
        setToast({ type: "error", message: data.error });
      }
    ...
    <DashboardLayout role="super_admin">
      <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)] mt-1">
              Create and manage fundraising campaigns for approved ventures
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Total", value: counts.all, icon: Megaphone, color: "text-[var(--brand-orange)]" },
            { label: "Active", value: counts.active, icon: Play, color: "text-emerald-400" },
            { label: "Draft", value: counts.draft, icon: Edit3, color: "text-slate-400" },
            { label: "Closed", value: counts.closed, icon: XCircle, color: "text-rose-400" },
          ].map((s, i) => (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
              <div className="sticky top-0 bg-[var(--surface-1)] flex items-center justify-between px-6 py-4 border-b border-[var(--border-primary)]">
                <h3 className="text-sm font-black text-[var(--text-primary)] uppercase">Create Campaign</h3>
                <button onClick={() => setShowCreate(false)} className="p-1.5 rounded-lg hover:bg-[var(--surface-3)]">✕</button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Venture *</label>
                  <select value={form.venture_id} onChange={e => setForm({ ...form, venture_id: e.target.value })}
                    ...
                    {ventures.map(v => (
                      <option key={v.id} value={v.id}>{v.name} ({v.industry || "N/A"})</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Campaign Name *</label>
                  <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                    placeholder="e.g. NovaSpark Pre-Seed Round"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Target Raise ($)</label>
                    <input type="number" value={form.target_raise} onChange={e => setForm({ ...form, target_raise: e.target.value })}
                      placeholder="250000"
                      className="w-full mt-1 px-3 py-2.5 bg-[var(--surface-2)] border border-[var(--border-primary)] rounded-xl text-xs font-bold text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] outline-none focus:border-[var(--brand-orange)]/60" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Min Investment ($)</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Opening Date</label>
                    <input type="date" value={form.opening_date} onChange={e => setForm({ ...form, opening_date: e.target.value })}
                      className="w-full mt-1 px-3 py-2.5 bg-[var(--surface-2)] border border-[var(--border-primary)] rounded-xl text-xs font-bold text-[var(--text-primary)] outline-none focus:border-[var(--brand-orange)]/60" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Closing Date</label>
                    <input type="date" value={form.closing_date} onChange={e => setForm({ ...form, closing_date: e.target.value })}
                      className="w-full mt-1 px-3 py-2.5 bg-[var(--surface-2)] border border-[var(--border-primary)] rounded-xl text-xs font-bold text-[var(--text-primary)] outline-none focus:border-[var(--brand-orange)]/60" />
                  </div>
                </div>
                <div>
                  <label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Visibility</label>
              ...
              <div className="flex justify-end gap-2 px-6 pb-5">
                <AppButton variant="secondary" size="sm" onClick={() => setShowCreate(false)}>Cancel</AppButton>
                <AppButton variant="primary" size="sm" icon={Plus} onClick={handleCreate}>Create Draft</AppButton>
              </div>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm font-bold text-[var(--text-secondary)]">No campaigns yet</p>
            <p className="text-xs text-[var(--text-tertiary)] mt-1">Create your first fundraising campaign</p>
          </div>
        ) : (
                ...
                <AppCard key={c.id} padding="md">
                  <div className="space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] text-[var(--text-tertiary)]">{VIS_LABELS[c.visibility]}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-4 text-[10px] text-[var(--text-tertiary)]">
                      {c.investor_count > 0 && <span className="flex items-center gap-1"><Users className="w-3 h-3"/>{c.investor_count} investors</span>}
                      {c.active_dd_count > 0 && <span className="flex items-center gap-1"><TrendingUp className="w-3 h-3"/>{c.active_dd_count} in DD</span>}
                    ...
                    <div className="flex gap-2 pt-1">
                      {c.status === "draft" && (
                        <AppButton variant="primary" size="sm" icon={Play} onClick={() => handleStatusChange(c.id, "active")}>Publish</AppButton>
                      )}
                      {c.status === "active" && (
                        <>
                          <AppButton variant="secondary" size="sm" icon={DollarSign} onClick={() => handleUpdateRaised(c.id)}>Update Raised</AppButton>
                          <AppButton variant="secondary" size="sm" icon={Pause} onClick={() => handleStatusChange(c.id, "paused")}>Pause</AppButton>
                          <AppButton variant="secondary" size="sm" icon={XCircle} onClick={() => handleStatusChange(c.id, "closed")}>Close</AppButton>
                        </>
                      )}
                      {c.status === "paused" && (
                        <>
                          <AppButton variant="primary" size="sm" icon={Play} onClick={() => handleStatusChange(c.id, "active")}>Resume</AppButton>
                          <AppButton variant="secondary" size="sm" icon={XCircle} onClick={() => handleStatusChange(c.id, "closed")}>Close</AppButton>
                        </>
                      )}
                    </div>
