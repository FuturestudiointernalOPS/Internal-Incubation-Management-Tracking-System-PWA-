  ...
  return(<><div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-3"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase">{t("investorAdmin.overview.introductionRequestsCount",{count:data.pipelines.filter(p=>p.stage==='meeting_requested').length})}</h3>
          ...
          data.pipelines.filter(p=>p.stage==='meeting_requested').map(p=>(<AppCard key={p.id} padding="md" hover onClick={()=>setDetail(p)}>
            <div className="cursor-pointer flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-1 rounded text-[10px] font-bold uppercase bg-amber-500/10 text-amber-400">{STAGE_LABELS[p.stage]||p.stage}</span>
              </div>
            </div>
          </AppCard>))
        }
        ...
        {data.pipelines.filter(p=>p.stage!=='meeting_requested').map(p=>(<AppCard key={p.id} padding="md"><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-[var(--text-primary)]">{p.venture_name||"—"}</p><p className="text-[10px] text-[var(--text-secondary)]">{p.investor_name} · {p.organization_name||""}</p></div><div className="flex items-center gap-2">{p.investment_amount&&<span className="text-xs font-bold text-emerald-400">${Number(p.investment_amount).toLocaleString()}</span>}<span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${STAGE_COLORS[p.stage]||"bg-slate-500/10 text-slate-400"}`}>{STAGE_LABELS[p.stage]||p.stage}</span></div></div></AppCard>))}
      </div>
    </div>

    {/* Requests */}
    <div className="space-y-3 mt-6"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase">{t("investorAdmin.overview.informationRequestsCount",{count:data.requests.length})}</h3>{data.requests.length===0?<p className="text-xs text-[var(--text-tertiary)] py-4">{t("investorAdmin.overview.noRequests")}</p>:data.requests.map(r=>(<AppCard key={r.id} padding="md"><div className="flex items-start justify-between"><div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${REQ_CAT_COLORS[r.category]||REQ_CAT_COLORS.general}`}>{r.category}</span><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${r.status==="pending"?"bg-amber-500/10 text-amber-400":r.status==="responded"?"bg-emerald-500/10 text-emerald-400":"bg-slate-500/10 text-slate-400"}`}>{r.status}</span></div><p className="text-sm font-bold text-[var(--text-primary)]">{r.title}</p><p className="text-[10px] text-[var(--text-secondary)]">{r.venture_name} · {r.investor_name}</p>{r.response_text&&<p className="text-[10px] text-emerald-400 mt-1">{t("investorAdmin.overview.response",{text:r.response_text})}</p>}</div><span className="text-[10px] text-[var(--text-tertiary)] shrink-0">{new Date(r.created_at).toLocaleDateString()}</span></div></AppCard>))}</div>
        ...
        <div className="relative w-full max-w-lg bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[[t("investorAdmin.overview.venture"),detail.venture_name||"—"],[t("investorAdmin.overview.investor"),detail.investor_name||"—"],[t("investorAdmin.overview.organization"),detail.organization_name||"—"],[t("investorAdmin.overview.stage"),STAGE_LABELS[detail.stage]||detail.stage],[t("investorAdmin.overview.email"),detail.email||"—"],[t("investorAdmin.overview.date"),new Date(detail.stage_changed_at).toLocaleDateString()]].map(([l,v],i)=>(<div key={i} className="p-3 rounded-xl bg-[var(--surface-3)]"><p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest">{l}</p><p className="text-xs font-bold text-[var(--text-primary)] mt-1">{v}</p></div>))}
            </div>
            {/* Investor Profile */}
