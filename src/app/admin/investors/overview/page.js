...
export default function AdminInvestorOverview() {
  ...
  return(<DashboardLayout role="super_admin"><div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="space-y-3"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase">{t("investorAdmin.overview.dueDiligenceCount",{count:data.workspaces.length})}</h3>{data.workspaces.length===0?<p className="text-xs text-[var(--text-tertiary)] py-4">{t("investorAdmin.overview.noActiveDd")}</p>:data.workspaces.map(w=>(<AppCard key={w.id} padding="md"><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-[var(--text-primary)]">{w.venture_name||"—"}</p><p className="text-[10px] text-[var(--text-secondary)]">{w.investor_name} · {w.organization_name||""}</p></div><span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${w.status==="active"?"bg-purple-500/10 text-purple-400":"bg-emerald-500/10 text-emerald-400"}`}>{w.status}</span></div></AppCard>))}</div>

      <div className="space-y-3"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase">{t("investorAdmin.overview.introductionRequestsCount",{count:data.pipelines.filter(p=>p.stage==='meeting_requested').length})}</h3>
        {data.pipelines.filter(p=>p.stage==='meeting_requested').length===0?<p className="text-xs text-[var(--text-tertiary)] py-4">{t("investorAdmin.overview.noPendingIntroductionRequests")}</p>:
    ...
    {/* Requests */}
    <div className="space-y-3 mt-6"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase">{t("investorAdmin.overview.informationRequestsCount",{count:data.requests.length})}</h3>{data.requests.length===0?<p className="text-xs text-[var(--text-tertiary)] py-4">{t("investorAdmin.overview.noRequests")}</p>:data.requests.map(r=>(<AppCard key={r.id} padding="md"><div className="flex items-start justify-between"><div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${REQ_CAT_COLORS[r.category]||REQ_CAT_COLORS.general}`}>{r.category}</span><span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${r.status==="pending"?"bg-amber-500/10 text-amber-400":r.status==="responded"?"bg-emerald-500/10 text-emerald-400":"bg-slate-500/10 text-slate-400"}`}>{r.status}</span></div><p className="text-sm font-bold text-[var(--text-primary)]">{r.title}</p><p className="text-[10px] text-[var(--text-secondary)]">{r.venture_name} · {r.investor_name}</p>{r.response_text&&<p className="text-[10px] text-emerald-400 mt-1">{t("investorAdmin.overview.response",{text:r.response_text})}</p>}</div><span className="text-[10px] text-[var(--text-tertiary)] shrink-0">{new Date(r.created_at).toLocaleDateString()}</span></div></AppCard>))}</div>
        ...
        <div className="relative w-full max-w-lg bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[[t("investorAdmin.overview.venture"),detail.venture_name||"—"],[t("investorAdmin.overview.investor"),detail.investor_name||"—"],[t("investorAdmin.overview.organization"),detail.organization_name||"—"],[t("investorAdmin.overview.stage"),STAGE_LABELS[detail.stage]||detail.stage],[t("investorAdmin.overview.email"),detail.email||"—"],[t("investorAdmin.overview.date"),new Date(detail.stage_changed_at).toLocaleDateString()]].map(([l,v],i)=>(<div key={i} className="p-3 rounded-xl bg-[var(--surface-3)]"><p className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-widest">{l}</p><p className="text-xs font-bold text-[var(--text-primary)] mt-1">{v}</p></div>))}
            </div>
            {/* Investor Profile */}
            {(detail.industries?.length > 0 || detail.countries?.length > 0 || detail.startup_stages?.length > 0) && (
              <div className="p-3 rounded-xl bg-[var(--surface-2)] border border-[var(--border-primary)] space-y-2">
                <p className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">{t("investorAdmin.overview.investorProfile")}</p>
                <div className="grid grid-cols-2 gap-2">
                  {detail.industries?.length > 0 && <div><p className="text-[7px] text-[var(--text-tertiary)] uppercase">{t("investorAdmin.overview.industries")}</p><p className="text-[10px] font-bold text-[var(--text-primary)]">{(detail.industries||[]).join(", ")}</p></div>}
                  {detail.countries?.length > 0 && <div><p className="text-[7px] text-[var(--text-tertiary)] uppercase">{t("investorAdmin.overview.countries")}</p><p className="text-[10px] font-bold text-[var(--text-primary)]">{(detail.countries||[]).join(", ")}</p></div>}
                  {detail.startup_stages?.length > 0 && <div><p className="text-[7px] text-[var(--text-tertiary)] uppercase">{t("investorAdmin.overview.stages")}</p><p className="text-[10px] font-bold text-[var(--text-primary)]">{(detail.startup_stages||[]).join(", ")}</p></div>}
                  {(detail.ticket_size_min || detail.ticket_size_max) && <div><p className="text-[7px] text-[var(--text-tertiary)] uppercase">{t("investorAdmin.overview.ticket")}</p><p className="text-[10px] font-bold text-[var(--text-primary)]">${detail.ticket_size_min||"0"}–${detail.ticket_size_max||"∞"}</p></div>}
                </div>
              </div>
            )}
            {detail.notes ? (
              <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
                <p className="text-[9px] font-black text-amber-400 uppercase tracking-widest mb-1 flex items-center gap-1"><MessageSquare className="w-3 h-3"/> {t("investorAdmin.overview.investorMessage")}</p>
            ...
            <div className="p-4 rounded-xl bg-[var(--surface-2)] border border-[var(--border-primary)]">
              <p className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-1 flex items-center gap-1"><UserPlus className="w-3 h-3"/> {t("investorAdmin.overview.actions")}</p>
