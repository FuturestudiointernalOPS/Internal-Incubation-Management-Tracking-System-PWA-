...
export default function VentureKnowledgePage() {
  ...
  const handleBookmark = async (resourceId) => {
    ...
    const d = await res.json();
    if (d.success) { notify(d.bookmarked ? "Bookmarked" : "Removed"); fetchAll(); }
  };
  ...
  const createResource = async () => {
    if (!crForm.title.trim()) { notify("Title required", "error"); return; }
    setSaving(true);
    try {
      const res = await fetch(`/api/ventures/${id}/knowledge`, {
        method: "POST", headers: { "Content-Type": "application/json" },
      ...
      const d = await res.json();
      if (d.success) { notify("Resource created"); setShowCreateModal(false); setCrForm({ title: "", description: "", resource_type: "article", category_id: "", url: "", tags: "" }); fetchAll(); }
      ...
      <div className="space-y-8 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-black text-[var(--text-primary)] flex items-center gap-3">
              <BookOpen className="w-6 h-6 text-[var(--brand-orange)]" /> Knowledge Hub
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">{resources.length} resources · {categories.length} categories</p>
          </div>
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()}
              placeholder="Search resources..." className="w-full pl-12 pr-4 py-3 bg-secondary border border-[var(--border-primary)] rounded-xl text-sm font-bold text-[var(--text-primary)] outline-none focus:border-[var(--brand-orange)]" />
          </div>
          <button onClick={handleSearch} className="px-4 py-3 bg-[var(--brand-orange)]/10 text-[var(--brand-orange)] rounded-xl text-[9px] font-black uppercase tracking-widest hover:brightness-110">Search</button>
        </div>
        ...
        <div className="flex gap-1 border-b border-[var(--border-primary)]">
          {[
            ...
            { id: "progress", label: `Learning`, icon: TrendingUp },
          ].map((tab) => {
            const Icon = tab.icon;
            return (
            ...
            {activeView === "progress" ? (
              <div className="col-span-full space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="p-4 rounded-2xl bg-tertiary border border-[var(--border-primary)]">
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Hours Learned</p>
                ...
                <div className="card">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Overall Learning Progress</span>
                ...
                {learningPaths.length > 0 && (
                  <div className="card">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Learning Paths</h3>
                        ...
                        <div key={p.id} className="p-4 rounded-xl bg-tertiary border border-[var(--border-primary)]">
                          <div className="flex items-center justify-between mb-2">
                            <div>
                              ...
                              <p className="text-[8px] text-slate-500 capitalize">{p.level} · {p.estimated_hours}h estimated</p>
                            </div>
                ...
                {(learningProgress?.pending_resources || []).length > 0 && (
                  <div className="card">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Continue Learning</h3>
                      ...
                      {learningProgress.pending_resources.map((r) => (
                        <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl bg-tertiary border border-[var(--border-primary)] cursor-pointer hover:border-[var(--brand-orange)]/30" onClick={() => loadResource(r.id)}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[8px] text-slate-500">{r.category_name || ""} · Last viewed {r.last_viewed_at ? new Date(r.last_viewed_at).toLocaleDateString() : ""}</p>
                          </div>
              ...
              <div className="col-span-full text-center py-16">
                ...
                <p className="text-sm text-slate-500">{activeView === "bookmarks" ? "No bookmarks yet" : "No resources found"}</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {r.is_featured && <span className="text-[7px] font-black px-1 py-0.5 rounded bg-amber-500/10 text-amber-400">Featured</span>}
                        </div>
                        ...
                        <div className="flex items-center gap-3 mt-2 text-[8px] text-slate-500">
                          ...
                          {r.estimated_minutes && <span className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" />{r.estimated_minutes} min</span>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-3xl p-8 max-h-[80vh] overflow-y-auto space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedResource.estimated_minutes && <span className="text-[8px] font-bold px-2 py-1 rounded bg-slate-500/10 text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3" />{selectedResource.estimated_minutes} min</span>}
              <span className="text-[8px] font-bold px-2 py-1 rounded bg-slate-500/10 text-slate-400 flex items-center gap-1"><Eye className="w-3 h-3" />{selectedResource.view_count || 0} views</span>
              {selectedResource.is_completed && <span className="text-[8px] font-bold px-2 py-1 rounded bg-emerald-500/10 text-emerald-400">✓ Completed</span>}
            </div>
            ...
            <div className="flex gap-3 pt-2 border-t border-[var(--border-primary)]">
              <button onClick={() => { handleBookmark(selectedResource.id); setSelectedResource((p) => ({ ...p, is_bookmarked: !p.is_bookmarked })); }}
                ...
                <Bookmark className={`w-3 h-3 ${selectedResource.is_bookmarked ? "fill-current" : ""}`} /> {selectedResource.is_bookmarked ? "Bookmarked" : "Bookmark"}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-3xl p-8 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-black text-[var(--text-primary)]">Add Resource</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={crForm.resource_type} onChange={(e) => setCrForm((p) => ({ ...p, resource_type: e.target.value }))} className="w-full bg-primary border border-[var(--border-primary)] rounded-xl px-4 py-3 text-sm font-bold text-[var(--text-primary)] outline-none">
                    <option value="article">Article</option><option value="video">Video</option><option value="pdf">PDF</option>
                    <option value="template">Template</option><option value="checklist">Checklist</option><option value="presentation">Presentation</option>
                    <option value="external_link">External Link</option><option value="course">Course</option><option value="case_study">Case Study</option>
                  </select>
                </div>
                <div>
              ...
              <div>
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">URL (for external resources)</label>
              ...
              <div>
                ...
                <input value={crForm.tags} onChange={(e) => setCrForm((p) => ({ ...p, tags: e.target.value }))} placeholder="fundraising, pitch, investors" className="w-full bg-primary border border-[var(--border-primary)] rounded-xl px-4 py-3 text-sm font-bold text-[var(--text-primary)] outline-none" />
              </div>
            </div>
            <div className="flex gap-3">
