...
export default function SuperAdminComms() {
  ...
  useEffect(() => {
    async function initAuth() {
      try {
        ...
        // Fallback — proceed with empty user
      }
      fetchData();
    }
    initAuth();
  ...
  const fetchData = async () => {
    try {
      const msgRes = await fetch("/api/internal-comms");
      const msgData = await msgRes.json();
      if (msgData.success) setMessages(msgData.messages);

      const progRes = await fetch("/api/pm/full-state"); // Reuse existing state fetcher if possible
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start gap-10">
          <div className="space-y-4">
            ...
            <p className="text-slate-400 font-bold max-w-xl opacity-70">
              Execute direct communication across the multi-tenant architecture.
            </p>
          </div>
          <button
          ...
          <div className="xl:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className="ios-card bg-white/[0.01] border-white/5 p-8 flex items-start gap-8 group hover:border-[#FF6600]/20 transition-all"
                >
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center border ${msg.priority === "high" ? "bg-rose-500/10 text-rose-500 border-rose-500/20" : "bg-[#FF6600]/10 text-[#FF6600] border-[#FF6600]/20"}`}
                  >
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <span
                          className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${msg.target_type === "all" ? "bg-[#FF6600]/80/20 text-indigo-400" : "bg-white/5 text-slate-500"}`}
                        >
                          {msg.target_type}
                        </span>
                    ...
                    <div className="pt-4 flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-600">
                      <span className="text-[#FF6600]">
                        To: {msg.target_id || "All Personnel"}
                      </span>
                      <span className="opacity-20">•</span>
                      <span>
                        Status: {msg.is_read ? "Intercepted" : "Transmitting"}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {messages.length === 0 && (
                <div className="ios-card border-dashed py-40 text-center italic text-slate-700 text-[11px] uppercase tracking-[0.4em]">
                  No Active Signals in Logic Log...
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              <div className="ios-card bg-[#FF6600]/5 border-[#FF6600]/10 p-8">
                <h4 className="text-[11px] font-black text-[#FF6600] uppercase tracking-widest mb-6">
                  Program Clusters
                </h4>
                <div className="space-y-4">
                  {programs.map((prog) => (
              ...
              <div className="ios-card bg-white/[0.01] border-white/5 p-8">
                ...
                <div className="space-y-4 text-white font-black text-[12px] italic uppercase tracking-tighter">
                  <p className="hover:text-[#FF6600] cursor-pointer transition-colors">
                    Program Managers
                  </p>
                  <p className="hover:text-[#FF6600] cursor-pointer transition-colors">
                    Group Leaders
