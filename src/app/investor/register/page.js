    ...
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center space-y-6">
          ...
          <h1 className="text-2xl font-black text-[var(--text-primary)] uppercase">Registration Submitted</h1>
          <p className="text-sm text-[var(--text-secondary)]">
            Your investor account is pending review. You'll be notified once approved.
          </p>
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center p-4">
      <div className="w-full max-w-lg space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black text-[var(--text-primary)] uppercase tracking-tighter">Investor Registration</h1>
          <p className="text-sm text-[var(--text-secondary)]">
            Join Future Studio's investment ecosystem
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="border-t border-[var(--border-primary)] pt-4 space-y-3">
            <p className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Organization (Optional)</p>
            <div className="relative">
              ...
              <input type="text" name="organization_name" value={form.organization_name} onChange={handleChange}
                placeholder="Organization / VC Firm Name"
                className="w-full pl-11 pr-4 py-3 bg-[var(--surface-2)] border border-[var(--border-primary)] rounded-xl text-sm font-bold text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] outline-none focus:border-[var(--brand-orange)]/60" />
            </div>
            <textarea name="biography" value={form.biography} onChange={handleChange}
              rows={2} placeholder="Brief investment focus / biography"
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting...</> : "Register as Investor"}
          </button>

          <p className="text-center text-[10px] text-[var(--text-tertiary)]">
            Already registered?{" "}
            <button type="button" onClick={() => router.push("/login")}
              className="text-[var(--brand-orange)] font-bold hover:underline">Log in</button>
          </p>
        </form>
      </div>
    </div>
