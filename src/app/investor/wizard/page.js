      ...
      <div className="w-full max-w-2xl space-y-6">
        ...
        <div className="bg-[var(--surface-1)] border border-[var(--border-primary)] rounded-2xl p-6 space-y-4 min-h-[320px]">
          ...
          {step === 2 && (<div className="space-y-4">
            <div><label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">{t("wizard.industries")} *</label><div className="flex flex-wrap gap-1.5 mt-1.5">{INDUSTRIES.map(ind=>(<button key={ind} onClick={()=>toggleArray(industries,setIndustries,ind)} className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase ${industries.includes(ind)?"bg-[var(--brand-orange)] text-white":"bg-[var(--surface-3)] text-[var(--text-secondary)]"}`}>{ind}</button>))}</div></div>
            <div><label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-1"><MapPin className="w-3 h-3"/> {t("wizard.countries")} *</label><div className="flex flex-wrap gap-1.5 mt-1.5">{COUNTRIES.map(c=>(<button key={c} onClick={()=>toggleArray(countries,setCountries,c)} className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase ${countries.includes(c)?"bg-[var(--brand-orange)] text-white":"bg-[var(--surface-3)] text-[var(--text-secondary)]"}`}>{c}</button>))}</div></div>
            <div><label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest flex items-center gap-1"><TrendingUp className="w-3 h-3"/> {t("wizard.stages")}</label><div className="flex flex-wrap gap-1.5 mt-1.5">{STAGES.map(s=>(<button key={s} onClick={()=>toggleArray(stages,setStages,s)} className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase ${stages.includes(s)?"bg-[var(--brand-orange)] text-white":"bg-[var(--surface-3)] text-[var(--text-secondary)]"}`}>{s}</button>))}</div></div>
            <div><label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest"><DollarSign className="w-3 h-3 inline"/> {t("wizard.ticketSize")}</label><div className="grid grid-cols-2 gap-3 mt-1.5"><input type="number" value={ticketMin} onChange={e=>setTicketMin(e.target.value)} placeholder={t("wizard.min")} className="px-4 py-2.5 bg-[var(--surface-2)] border rounded-xl text-sm font-bold outline-none"/><input type="number" value={ticketMax} onChange={e=>setTicketMax(e.target.value)} placeholder={t("wizard.max")} className="px-4 py-2.5 bg-[var(--surface-2)] border rounded-xl text-sm font-bold outline-none"/></div></div>
          </div>)}

          {step === 3 && (<div className="space-y-3">
            <div><label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">{t("wizard.investExperience")}</label><textarea value={investmentExperience} onChange={e=>setInvestmentExperience(e.target.value)} rows={3} placeholder={t("wizard.investExperienceDesc")} className="w-full px-4 py-3 bg-[var(--surface-2)] border rounded-xl text-sm font-bold text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] outline-none resize-none"/></div>
            <div><label className="text-[9px] font-black text-[var(--text-secondary)] uppercase tracking-widest">{t("wizard.priorPortfolio")}</label><textarea value={priorInvestments} onChange={e=>setPriorInvestments(e.target.value)} rows={2} placeholder={t("wizard.priorPortfolioDesc")} className="w-full px-4 py-3 bg-[var(--surface-2)] border rounded-xl text-sm font-bold text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] outline-none resize-none"/></div>
          </div>)}
