...
const pickInvestedPipeline = (d) =>
  d?.success ? (d.pipeline || []).filter((p) => p.stage === "invested") : [];
const pickDecisionTrace = (d) =>
  d?.success
    ? { decisions: d.decisions || [], stats: d.stats || {} }
    : EMPTY_DECISION_TRACE;
const pickMeetings = (d) => (d?.success ? d.meetings || [] : []);
const pickNotes = (d) => (d?.success ? d.notes || [] : []);
const pickUpdates = (d) => (d?.success ? d.updates || [] : []);
const pickKpis = (d) => (d?.success ? d.kpis || [] : []);
...
export default function PortfolioPage() {
  ...
  const addNote = async () => {
    ...
    try {
      ...
      if (res.ok) { setNewNote(""); refreshNotes(); }
    } catch (_) {}
  };
      ...
      <>
        <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
          ...
          {activeTab==="overview"&&(<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <AppCard padding="lg"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase mb-4 flex items-center gap-2"><FileText className="w-4 h-4 text-[var(--brand-orange)]"/> {t("investorMisc.portfolio.recentUpdates")}</h3>
              {updates.length===0?<p className="text-xs text-[var(--text-tertiary)] text-center py-8">{t("investorMisc.portfolio.noUpdatesYet")}</p>:<div className="space-y-3">{updates.slice(0,3).map(u=>(
                <div key={u.id} className="p-3 rounded-lg bg-[var(--surface-3)]"><div className="flex items-center justify-between mb-1"><p className="text-xs font-bold text-[var(--text-primary)]">{u.title}</p><span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-[var(--brand-orange)]/10 text-[var(--brand-orange)]">{u.update_type}</span></div><p className="text-[10px] text-[var(--text-secondary)]">{u.content.substring(0,80)}{u.content.length>80?"...":""}</p><p className="text-[10px] font-medium text-[var(--text-tertiary)] mt-1">{new Date(u.created_at).toLocaleDateString()}</p></div>
              ))}</div>}
            </AppCard>
          ...
          {activeTab==="updates"&&(<div className="space-y-4">
            ...
            {updates.length===0&&!showUpdateForm?<div className="text-center py-12"><FileText className="w-10 h-10 text-[var(--text-tertiary)] mx-auto mb-3"/><p className="text-sm font-bold text-[var(--text-secondary)]">{t("investorMisc.portfolio.noUpdatesYet")}</p></div>:<div className="space-y-3">{updates.map(u=>(<AppCard key={u.id} padding="md"><div className="flex items-start gap-3"><div className="w-2 h-2 rounded-full mt-1.5 shrink-0 bg-[var(--brand-orange)]"/><div><div className="flex items-center gap-2 mb-1"><p className="text-sm font-bold text-[var(--text-primary)]">{u.title}</p><span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-[var(--brand-orange)]/10 text-[var(--brand-orange)]">{u.update_type}</span></div><p className="text-xs text-[var(--text-secondary)]">{u.content}</p><p className="text-[10px] font-medium text-[var(--text-tertiary)] mt-2">{new Date(u.created_at).toLocaleDateString()}</p></div></div></AppCard>))}</div>}
          </div>)}
          ...
          {activeTab==="meetings"&&(<div className="space-y-4">
            ...
            {meetings.length===0?<div className="text-center py-12"><Calendar className="w-10 h-10 text-[var(--text-tertiary)] mx-auto mb-3"/><p className="text-sm font-bold text-[var(--text-secondary)]">{t("investorMisc.portfolio.noMeetingsYet")}</p></div>:<div className="space-y-3">{meetings.map(m=>(<AppCard key={m.id} padding="md"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Calendar className="w-8 h-8 text-[var(--brand-orange)]"/><div><p className="text-sm font-bold text-[var(--text-primary)]">{m.title}</p><p className="text-[10px] text-[var(--text-secondary)]">{new Date(m.start_time).toLocaleString()} · {m.location}{m.end_time?` → ${new Date(m.end_time).toLocaleString()}`:""}</p></div></div><span className="px-2 py-1 rounded text-[10px] font-bold uppercase bg-[var(--brand-orange)]/10 text-[var(--brand-orange)]">{t("investorMisc.portfolio.scheduled")}</span></div></AppCard>))}</div>}
          </div>)}
    ...
    <>
      <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[{label:t("investorMisc.portfolio.invested"),value:pipeline.length,icon:Target,color:"text-emerald-400"},{label:t("investorMisc.portfolio.totalCapital"),value:`$${(stats.total_capital||0).toLocaleString()}`,icon:DollarSign,color:"text-[var(--brand-orange)]"},{label:t("investorMisc.portfolio.decisions"),value:stats.total_decisions||0,icon:BarChart3,color:"text-purple-400"}].map((s,i)=>(<AppCard key={i} padding="md"><div className="flex items-center gap-3"><s.icon className={`w-5 h-5 ${s.color}`}/><div><p className="text-xl font-black text-[var(--text-primary)]">{s.value}</p><p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">{s.label}</p></div></div></AppCard>))}
        </div>
