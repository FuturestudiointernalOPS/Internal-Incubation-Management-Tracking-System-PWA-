      ...
      <>
        <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
          ...
          {activeTab==="overview"&&(<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <AppCard padding="lg"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase mb-4 flex items-center gap-2"><FileText className="w-4 h-4 text-[var(--brand-orange)]"/> {t("investorMisc.portfolio.recentUpdates")}</h3>
              {updates.length===0?<p className="text-xs text-[var(--text-tertiary)] text-center py-8">{t("investorMisc.portfolio.noUpdatesYet")}</p>:<div className="space-y-3">{updates.slice(0,3).map(update=>(
                <div key={update.id} className="p-3 rounded-lg bg-[var(--surface-3)]"><div className="flex items-center justify-between mb-1"><p className="text-xs font-bold text-[var(--text-primary)]">{update.title}</p><span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-[var(--brand-orange)]/10 text-[var(--brand-orange)]">{update.update_type}</span></div><p className="text-[10px] text-[var(--text-secondary)]">{update.content.substring(0,80)}{update.content.length>80?"...":""}</p><p className="text-[10px] font-medium text-[var(--text-tertiary)] mt-1">{new Date(update.created_at).toLocaleDateString()}</p></div>
              ))}</div>}
            </AppCard>
            <AppCard padding="lg"><h3 className="text-sm font-black text-[var(--text-primary)] uppercase mb-4 flex items-center gap-2"><Calendar className="w-4 h-4 text-purple-400"/> {t("investorMisc.portfolio.upcomingMeetings")}</h3>
              {upcomingMeetings.length===0?<p className="text-xs text-[var(--text-tertiary)] text-center py-8">{t("investorMisc.portfolio.noUpcomingMeetings")}</p>:<div className="space-y-3">{upcomingMeetings.slice(0,3).map(meeting=>(
                <div key={meeting.id} className="flex items-center gap-3 p-3 rounded-lg bg-[var(--brand-orange)]/5 border border-[var(--brand-orange)]/10"><Calendar className="w-4 h-4 text-[var(--brand-orange)]"/><div className="flex-1"><p className="text-xs font-bold text-[var(--text-primary)]">{meeting.title}</p><p className="text-[10px] text-[var(--text-secondary)]">{new Date(meeting.start_time).toLocaleDateString()} · {meeting.location}</p></div></div>
              ))}</div>}
            </AppCard>
          </div>)}
          ...
          {activeTab==="updates"&&(<div className="space-y-4">
            ...
            {updates.length===0&&!showUpdateForm?<div className="text-center py-12"><FileText className="w-10 h-10 text-[var(--text-tertiary)] mx-auto mb-3"/><p className="text-sm font-bold text-[var(--text-secondary)]">{t("investorMisc.portfolio.noUpdatesYet")}</p></div>:<div className="space-y-3">{updates.map(update=>(<AppCard key={update.id} padding="md"><div className="flex items-start gap-3"><div className="w-2 h-2 rounded-full mt-1.5 shrink-0 bg-[var(--brand-orange)]"/><div><div className="flex items-center gap-2 mb-1"><p className="text-sm font-bold text-[var(--text-primary)]">{update.title}</p><span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-[var(--brand-orange)]/10 text-[var(--brand-orange)]">{update.update_type}</span></div><p className="text-xs text-[var(--text-secondary)]">{update.content}</p><p className="text-[10px] font-medium text-[var(--text-tertiary)] mt-2">{new Date(update.created_at).toLocaleDateString()}</p></div></div></AppCard>))}</div>}
          </div>)}
          ...
          {activeTab==="meetings"&&(<div className="space-y-4">
            ...
            {meetings.length===0?<div className="text-center py-12"><Calendar className="w-10 h-10 text-[var(--text-tertiary)] mx-auto mb-3"/><p className="text-sm font-bold text-[var(--text-secondary)]">{t("investorMisc.portfolio.noMeetingsYet")}</p></div>:<div className="space-y-3">{meetings.map(meeting=>(<AppCard key={meeting.id} padding="md"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Calendar className="w-8 h-8 text-[var(--brand-orange)]"/><div><p className="text-sm font-bold text-[var(--text-primary)]">{meeting.title}</p><p className="text-[10px] text-[var(--text-secondary)]">{new Date(meeting.start_time).toLocaleString()} · {meeting.location}{meeting.end_time?` → ${new Date(meeting.end_time).toLocaleString()}`:""}</p></div></div><span className="px-2 py-1 rounded text-[10px] font-bold uppercase bg-[var(--brand-orange)]/10 text-[var(--brand-orange)]">{t("investorMisc.portfolio.scheduled")}</span></div></AppCard>))}</div>}
          </div>)}
