...
export default function PMProfile() {
  ...
  useEffect(() => {
    ...
    setUser(sessionUser);
    const identifier =
      sessionUser?.cid || sessionUser?.id || sessionUser?.email;
    if (identifier) {
    ...
    <DashboardLayout role="program_manager" activeTab="profile">
      <div className="max-w-4xl mx-auto space-y-12">
        <header className="border-b border-white/5 pb-10">
          ...
          <p className="text-slate-500 font-bold mt-4 opacity-70">
            {t("navigation.profile")}
          </p>
        </header>
            ...
            <div className="ios-card bg-[#0d0d18] border-white/5 !p-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
                    <User className="w-3 h-3" /> {t("pm.teamOverview")}
                  </p>
                  <input
                    id="edit_name_field"
                ...
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
                    <Mail className="w-3 h-3" /> {t("navigation.communication")}
                  </p>
                ...
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
                    <Phone className="w-3 h-3" /> {t("common.search")}
                  </p>
                  <p className="text-sm font-bold text-white bg-white/5 p-4 rounded-xl border border-white/5">
                    {dbUser.phone || t("common.noResults")}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
                    <Calendar className="w-3 h-3" /> {t("time.created")}
                  </p>
                  <p className="text-sm font-bold text-white bg-white/5 p-4 rounded-xl border border-white/5">
                    {dbUser.dob || t("common.noResults")}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">
                    <MapPin className="w-3 h-3" /> {t("navigation.dashboard")}
                  </p>
                  <p className="text-sm font-bold text-white bg-white/5 p-4 rounded-xl border border-white/5">
                    {dbUser.address || t("common.noResults")}
                  </p>
                </div>
                <div className="space-y-1 sm:col-span-2">
                  ...
                  <p className="text-sm font-bold text-white bg-white/5 p-4 rounded-xl border border-white/5">
                    {dbUser.mother_name || t("common.noResults")}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row items-end gap-6">
                ...
                <button
                  onClick={async () => {
                    ...
                    const newName =
                      document.getElementById("edit_name_field").value;
