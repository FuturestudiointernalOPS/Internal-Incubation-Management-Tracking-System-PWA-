...
import { Send, MessageSquare, Search, Users, Briefcase, User, X } from "lucide-react";
...
export default function PmMessages() {
  ...
  const fetchProgramContacts = useCallback(async () => {
    if (!uid) return;
    try {
      ...
      // Collect all people from these programs
      const people = [];
      const seen = new Set();

      // Add SA + staff from contacts
      const contactRes = await fetch("/api/contacts");
      const contactData = await contactRes.json();
      if (contactData.success) {
        const admins = contactData.contacts.filter(
          (c) => c.status === "active" &&
            ["super_admin", "program_manager", "teacher", "staff"].includes(c.role),
        );
        admins.forEach((p) => {
          const pid = p.cid || p.id;
          if (!seen.has(pid)) { seen.add(pid); people.push(p); }
        });
      }

      // Add participants from each program
        ...
        try {
          ...
          if (stateData.success) {
            (stateData.participants || []).forEach((p) => {
              ...
              if (!seen.has(pid)) {
                seen.add(pid);
                people.push({ ...p, role: "participant" });
              }
            });
          }
        } catch (e) { /* skip */ }
  ...
  }, [uid, fetchMessages, fetchProgramContacts]);
  ...
  const handleSend = async () => {
    ...
    if (sendMode === "individual") {
      ...
      payload = { sender_id: uid, recipient_id: composeRecipient, target_type: "individual", subject: composeBody.substring(0, 50), body: composeBody, priority: "normal" };
    } else if (sendMode === "program") {
      if (!composeProgram) return;
      payload = { sender_id: uid, target_type: "program", target_id: composeProgram, subject: composeBody.substring(0, 50), body: composeBody, priority: "normal" };
    }
    try {
      await fetch("/api/internal-comms", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      setShowCompose(false);
      setComposeRecipient(""); setComposeProgram(""); setComposeBody("");
      fetchMessages();
    ...
    <DashboardLayout role="program_manager">
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-lg font-black uppercase tracking-tight text-[var(--text-primary)]">Communication</h1>
            <p className="text-[10px] text-[var(--text-secondary)] mt-1">{messages.length} messages</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { setShowCompose(true); fetchProgramContacts(); }}
        ...
        <div className="space-y-2">
          {loading ? (
            <div className="text-center py-20"><div className="w-8 h-8 border-2 border-[var(--brand-orange)] border-t-transparent rounded-full animate-spin mx-auto" /></div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20"><MessageSquare className="w-10 h-10 text-[var(--text-secondary)] mx-auto mb-3 opacity-30" /><p className="text-[11px] font-bold text-[var(--text-secondary)]">No messages</p></div>
          ) : (
            filtered.map((msg) => (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-bold text-[var(--brand-orange)] uppercase tracking-wider">
                        {msg.sender_id === uid ? "Sent" : "Received"}
                      </span>
                      {msg.target_type && msg.target_type !== "individual" && (
                        <span className="text-[8px] font-bold text-blue-400 uppercase tracking-wider px-1.5 py-0.5 rounded bg-blue-500/10">
                          {msg.target_type === "program" ? "Program" : msg.target_type === "role" ? `To ${msg.target_id || "Group"}` : msg.target_type}
                        </span>
                      )}
                    </div>
          ...
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm" onClick={() => setShowCompose(false)}>
            <div className="w-full max-w-lg rounded-xl bg-[var(--bg-primary)] border border-[var(--border-primary)] p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex gap-2 p-1 rounded-lg bg-tertiary border border-[var(--border-primary)]">
                {[{ id: "individual", label: "Individual", icon: User }, { id: "program", label: "To Program", icon: Briefcase }].map((mode) => (
                  <button key={mode.id} onClick={() => setSendMode(mode.id)}
              ...
              {sendMode === "program" && (
                <select value={composeProgram} onChange={(e) => setComposeProgram(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-lg bg-tertiary border border-[var(--border-primary)] text-[11px] font-bold text-[var(--text-primary)] outline-none">
              ...
              <textarea placeholder="Message" value={composeBody} onChange={(e) => setComposeBody(e.target.value)} rows={4}
              ...
              <button onClick={handleSend} disabled={!composeBody}
                className="w-full py-3 bg-[var(--brand-orange)] text-black rounded-xl text-[10px] font-black uppercase tracking-wider disabled:opacity-30 hover:brightness-110 transition-all">Send Message</button>
            </div>
          </div>
        )}
      </div>
