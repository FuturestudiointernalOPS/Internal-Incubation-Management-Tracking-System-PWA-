...
const SUBMISSION_1 = { submission_id: 501, task_id: TASK_1, version: 2, file_url: null, file_name: null, notes: "v2", submitted_by_name: "Founder A", created_at: "2026-01-02T10:00:00Z", task_title: "Pitch deck", milestone_title: "Raise" };
const SUBMISSION_2 = { submission_id: 502, task_id: TASK_2, version: 1, file_url: null, file_name: null, notes: "v1", submitted_by_name: "Founder B", created_at: "2026-01-03T10:00:00Z", task_title: "Financial model", milestone_title: "Finance" };
...
function makeFakeDb() {
  const flags = {
    ...
    tasksByVenture: [
      { id: TASK_1, milestone_id: MILESTONE_1, journey_stage_id: STAGE_1 },
      { id: TASK_2, milestone_id: MILESTONE_2, journey_stage_id: STAGE_2 },
    ],
    taskRowById: {},
    queueRows: [SUBMISSION_1, SUBMISSION_2],
    submissionExists: true,
    submissionRows: [{ id: 501, task_id: TASK_1, version: 2, status: "submitted", notes: "v2" }],
    stageLookup: { [MILESTONE_1]: STAGE_1, [MILESTONE_2]: STAGE_2 },
  };
...
function resetFixtures() {
  ...
  mockDb.flags.submissionRows = [{ id: 501, task_id: TASK_1, version: 2, status: "submitted", notes: "v2" }];
  mockDb.flags.stageLookup = { [MILESTONE_1]: STAGE_1, [MILESTONE_2]: STAGE_2 };
  mockDb.flags.taskRowById = {
    [TASK_1]: { id: TASK_1, venture_id: VENTURE_DB_ID, milestone_id: MILESTONE_1, title: "Pitch deck", status: "review", review_required: 1 },
    [TASK_2]: { id: TASK_2, venture_id: VENTURE_DB_ID, milestone_id: MILESTONE_2, title: "Financial model", status: "review", review_required: 1 },
  };
...
describe("ventureScope helpers — scope matching rules", () => {
  test("venture_wide passes any task", () => {
    expect(isTaskInScope([{ scope_type: "venture_wide", responsibility_code: "coach" }], { id: TASK_1, milestone_id: MILESTONE_1 })).toBe(true);
  });

  test("milestone scope matches stringified milestone_id", () => {
    const scopes = [{ scope_type: "milestone", scope_ref_id: MILESTONE_1, responsibility_code: "coach" }];
    expect(isTaskInScope(scopes, { id: TASK_1, milestone_id: MILESTONE_1 })).toBe(true);
    expect(isTaskInScope(scopes, { id: TASK_2, milestone_id: MILESTONE_2 })).toBe(false);
    expect(isTaskInScope(scopes, { id: TASK_1, milestone_id: MILESTONE_2 })).toBe(false);
  });

  test("task scope is the only pass for a task without milestone/journey linkage", () => {
    const taskScope = [{ scope_type: "task", scope_ref_id: String(TASK_1), responsibility_code: "coach" }];
    expect(isTaskInScope(taskScope, { id: TASK_1, milestone_id: null })).toBe(true);
    expect(isTaskInScope(taskScope, { id: TASK_2, milestone_id: null })).toBe(false);
    expect(isTaskInScope([{ scope_type: "milestone", scope_ref_id: MILESTONE_1 }], { id: TASK_1, milestone_id: null })).toBe(false);
  });
  ...
  test("journey_stage scope matches the task context's stage", () => {
    ...
    expect(isTaskInScope(scopes, { id: TASK_1, milestone_id: MILESTONE_1, journey_stage_id: STAGE_1 })).toBe(true);
    expect(isTaskInScope(scopes, { id: TASK_2, milestone_id: MILESTONE_2, journey_stage_id: STAGE_2 })).toBe(false);
  });

  test("empty/null scopes and unknown scope types never match", () => {
    expect(isTaskInScope([], { id: TASK_1, milestone_id: MILESTONE_1 })).toBe(false);
    expect(isTaskInScope(null, { id: TASK_1, milestone_id: MILESTONE_1 })).toBe(false);
    expect(isTaskInScope([{ scope_type: "workstream", scope_ref_id: "w1" }], { id: TASK_1, milestone_id: MILESTONE_1 })).toBe(false);
  });

  test("lead_manager assignment implies Venture-wide reach even under a typed scope", () => {
    expect(hasVentureWideReach([{ scope_type: "milestone", scope_ref_id: MILESTONE_1, responsibility_code: "lead_manager" }])).toBe(true);
    expect(hasVentureWideReach([{ scope_type: "venture_wide", responsibility_code: "facilitator" }])).toBe(true);
    expect(hasVentureWideReach([{ scope_type: "milestone", scope_ref_id: MILESTONE_1, responsibility_code: "coach" }])).toBe(false);
  });
...
describe("POST submissions {action:'review'} — assignment-scope gate", () => {
  test("global role (super_admin) passes review without scope rows", async () => {
    ...
    const res = await reviewPOST(reviewRequest(TASK_1, REVIEW_BODY), { params: { id: "VNT-TEST", taskId: String(TASK_1) } });
    expect(res.status).toBe(200);
    expect(executed.some((query) => query.sql.includes("FROM venture_staff_assignments"))).toBe(false);
    expect(executed.some((query) => query.sql.includes("UPDATE venture_task_submissions"))).toBe(true);
  });
  ...
  test("venture_wide assignment passes review anywhere", async () => {
    ...
    const res = await reviewPOST(reviewRequest(TASK_2, REVIEW_BODY), { params: { id: "VNT-TEST", taskId: String(TASK_2) } });
    expect(res.status).toBe(200);
    expect(executed.some((query) => query.sql.includes("UPDATE venture_task_submissions"))).toBe(true);
  });

  test("milestone-scoped reviewer may review a task in that milestone", async () => {
    mockDb.flags.assignments = [{ scope_type: "milestone", scope_ref_type: "milestone", scope_ref_id: MILESTONE_1, responsibility_code: "coach" }];
    const res = await reviewPOST(reviewRequest(TASK_1, REVIEW_BODY), { params: { id: "VNT-TEST", taskId: String(TASK_1) } });
    expect(res.status).toBe(200);
  ...
  test("milestone-scoped reviewer is 403 on a task in another milestone (and writes nothing)", async () => {
    ...
    expect(executed.some((query) => query.sql.includes("UPDATE venture_task_submissions"))).toBe(false);
  });
  ...
  test("journey_stage-scoped reviewer is confined to that stage", async () => {
    ...
    const inStage = await reviewPOST(reviewRequest(TASK_1, REVIEW_BODY), { params: { id: "VNT-TEST", taskId: String(TASK_1) } });
    expect(inStage.status).toBe(200);
    const outStage = await reviewPOST(reviewRequest(TASK_2, REVIEW_BODY), { params: { id: "VNT-TEST", taskId: String(TASK_2) } });
    expect(outStage.status).toBe(403);
  });

  test("lead_manager assignment may review anywhere in the Venture (typed scope)", async () => {
    mockDb.flags.assignments = [{ scope_type: "milestone", scope_ref_type: "milestone", scope_ref_id: MILESTONE_1, responsibility_code: "lead_manager" }];
    const res = await reviewPOST(reviewRequest(TASK_2, REVIEW_BODY), { params: { id: "VNT-TEST", taskId: String(TASK_2) } });
    expect(res.status).toBe(200);
  });
...
describe("GET submissions/review-queue — assignment-scoped attention list", () => {
  test("global role sees the full queue", async () => {
    ...
    expect(executed.some((query) => query.sql.includes("FROM venture_staff_assignments"))).toBe(false);
    expect(executed.some((query) => query.sql.includes("SELECT t.id, t.milestone_id, m.journey_stage_id"))).toBe(false);
  });
  ...
  test("venture_wide assignment sees the full queue", async () => {
    ...
    expect(res.status).toBe(200);
    const data = await readJson(res);
    expect(data.items.map((item) => item.task_id)).toEqual([TASK_1, TASK_2]);
    expect(executed.some((query) => query.sql.includes("SELECT t.id, t.milestone_id, m.journey_stage_id"))).toBe(false);
  });
  ...
  test("milestone-scoped reviewer sees only that milestone's pending tasks", async () => {
    ...
    expect(data.items[0].task_id).toBe(TASK_1);
  ...
  test("journey_stage-scoped reviewer sees only tasks in that stage", async () => {
    ...
    expect(res.status).toBe(200);
    const data = await readJson(res);
    expect(data.items.map((item) => item.task_id)).toEqual([TASK_2]);
  });
