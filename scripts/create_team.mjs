...
for (const member of teamMembers) {
  ...
  const exists = await db.execute({ sql: "SELECT cid FROM contacts WHERE email = ? AND deleted_at IS NULL", args: [member.email] });
  if (exists.rows.length > 0) {
    console.log(`⏭️ ${member.name} already exists: ${exists.rows[0].cid}`);
    continue;
  }
  ...
  await db.execute({
    sql: `INSERT INTO contacts (cid, name, email, password, role, created_at, status)
          VALUES (?, ?, ?, ?, ?, NOW(), 'active')`,
    args: [cid, member.name, member.email, PASSWORD, member.role],
  });

  console.log(`✅ ${member.name} (${member.title}) — ${member.email} / ImpactOS2026!`);
}
